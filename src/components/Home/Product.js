    ...
    return (
        <div key={item.id} className='bg-white h-auto border-[1px] border-gray-200 py-8 z-30 hover:border-transparent shadow-none hover:shadow-testShadow duration-200 relative flex flex-col gap-4'>
            ...
            <div className='px-4 z-50 bg-white'>
                ...
                <div>
                    <p className='text-sm mb-1'>{item.description.substring(0, 70)}...</p>
                ...
                {
                    item?.stock !== 0 ?
                        <button id='button' className='w-full font-titleFont font-medium text-base bg-gradient-to-tr from-yellow-400 to-yellow-200 border hover:from-yellow-300 hover:to-yellow-border-yellow-500 hover:border-yellow-700 active:bg-gradient-to-b1 active:from-yellow-400 active:to-yellow-500 duration-200 py-2 rounded-md mt-3' onClick={() => handleAddtoCart(item)}>Add to Cart</button>
                        :
                        <button type="button" className="w-full font-titleFont font-medium text-base py-2 rounded-md mt-3 focus:outline-none rounded-md disabled:opacity-75 bg-gradient-to-tr from-yellow-400 to-yellow-200 border hover:from-yellow-300 hover:to-yellow-border-yellow-500 hover:border-yellow-700" disabled>Out of Stock</button>
                }
